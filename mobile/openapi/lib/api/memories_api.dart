...
class MemoriesApi {
  MemoriesApi([ApiClient? apiClient])
  ...
  Future<Response> addMemoryAssetsWithHttpInfo(
    ...
    final path = r'/memories/{id}/assets'.replaceAll('{id}', id);
  ...
  Future<Response> deleteMemoryWithHttpInfo(
    ...
    // ignore: prefer_const_declarations
    final path = r'/memories/{id}'.replaceAll('{id}', id);
  ...
  Future<List<MemoryResponseDto>?> searchMemories() async {
    ...
    if (response.body.isNotEmpty &&
      ...
      final responseBody = await _decodeBodyBytes(response);
      return (await apiClient.deserializeAsync(
              responseBody, 'List<MemoryResponseDto>') as List)
